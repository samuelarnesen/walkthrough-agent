...
class TDQN(nn.Module):
    ...
    def act(self, state, epsilon):
        with torch.no_grad():
            state = torch.LongTensor(state).unsqueeze(0).permute(1, 0, 2).to(self.device)
    ...
    def score_valid_options(self, state, options):

        with torch.no_grad():
            state = torch.LongTensor(state).unsqueeze(0).permute(1, 0, 2).to(self.device)
            q_t, q_o1, q_o2 = self.forward(state)
            t_probs = F.softmax(q_t, dim=1).squeeze(0)
            o1_probs = F.softmax(q_o1, dim=1).squeeze(0)
            o2_probs = F.softmax(q_o2, dim=1).squeeze(0)

            scores = []
            for t, o1, o2 in options:

                t_score = t_probs[t]
                o1_score = o1_probs[o1] if o1 != None else 0
                o2_score = o2_probs[o2] if o2 != None else 0

                score = float(int(t_score * 1000))
                if o1 != -1:
                    score += o1_score if o2 == -1 else o1_score * o2_score
                scores.append(score)

            sorted_options = [option for _, option in reversed(sorted(zip(scores, options)))]
...
class StateNetwork(nn.Module):
    ...
    def forward(self, obs):
        ...
        x_l, h_l = self.enc_look(obs[0, :, :])
        x_i, h_i = self.enc_inv(obs[1, :, :])
        x_o, h_o = self.enc_ob(obs[2, :, :])
        x_p, h_p = self.enc_preva(obs[3, :, :])
...
class PackedEncoderRNN(nn.Module):
    ...
    def forward(self, input, hidden=None):
        ...
        lengths = torch.tensor([torch.nonzero(n)[-1] + 1 for n in input], dtype=torch.long).to(self.device)

        # ALL THIS IS ADDED

        lengths, perm_idx = lengths.sort(0, descending=True)

        embedded = embedded.permute(1, 0, 2)[perm_idx].permute(1, 0, 2)

        #print(input.size(0), embedded.size(), hidden.size())
