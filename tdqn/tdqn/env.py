...
class JerichoEnv:
    def __init__(self, rom_path, vocab_rev, step_limit=None, templates=None):
        ...
        if templates != None:
            self.act_gen.templates = copy.deepcopy(templates)
    ...
    def walkthrough_step(self, action):
    ...
    def step(self, action):
        ...
        if type(ob) == type(""):
            action_valid = (action_valid and "You can't see any" not in ob and "You don't have that" not in ob and "You can't do that" not in ob)
    ...
    def find_valid_actions(self, action_list):
        return self.env.find_valid_actions(action_list)
